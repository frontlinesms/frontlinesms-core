...
public abstract class ImportExportDialogHandler implements
	...
	/**
	 * Thinlet Component Name: Checkbox to indicate whether a contact's "notes"
	 * field should be exported.
	 */
	...
	public ImportExportDialogHandler(UiGeneratorController uiController) {
		...
		this.groupMembershipDao = uiController.getFrontlineController()
	...
	public void filenameModified(String text) {
		boolean enableExport = FrontlineUtils.getFilenameWithoutFinalExtension(
				new File(text)).length() > 0;
		uiController.setEnabled(
				uiController.find(this.wizardDialog, COMPONENT_BT_DO_EXPORT),
				enableExport);
	}
	...
	protected boolean isChecked(String checkboxComponentName) {
		...
		Object cbComponent = uiController.find(wizardDialog,
				checkboxComponentName);
		assert (cbComponent != null) : "The checkbox component could not be found with name: "
				+ checkboxComponentName;
	 ...
	 * @param checkboxComponentName
	 */
	protected void addMarker(CsvRowFormat rowFormat, String marker,
			String checkboxComponentName) {
	 ...
	 * 
	 * @return {@link CsvRowFormat} for message, reflecting the settings in
	 *         {@link #wizardDialog}
	 */
	protected CsvRowFormat getRowFormatForMessage() {
		CsvRowFormat rowFormat = new CsvRowFormat();
		addMarker(rowFormat, CsvUtils.MARKER_MESSAGE_TYPE, COMPONENT_CB_TYPE);
		addMarker(rowFormat, CsvUtils.MARKER_MESSAGE_STATUS,
				COMPONENT_CB_STATUS);
		addMarker(rowFormat, CsvUtils.MARKER_MESSAGE_DATE, COMPONENT_CB_DATE);
		addMarker(rowFormat, CsvUtils.MARKER_MESSAGE_CONTENT,
				COMPONENT_CB_CONTENT);
	 ...
	 * 
	 * @return {@link CsvRowFormat} for contacts, reflecting the settings in
	 *         {@link #wizardDialog}
	 */
	protected CsvRowFormat getRowFormatForContact() {
		...
		addMarker(rowFormat, CsvUtils.MARKER_CONTACT_OTHER_PHONE,
				COMPONENT_CB_OTHER_PHONE);
		addMarker(rowFormat, CsvUtils.MARKER_CONTACT_EMAIL, COMPONENT_CB_EMAIL);
		addMarker(rowFormat, CsvUtils.MARKER_CONTACT_STATUS,
				COMPONENT_CB_STATUS);
		addMarker(rowFormat, CsvUtils.MARKER_CONTACT_NOTES, COMPONENT_CB_NOTES);
		addMarker(rowFormat, CsvUtils.MARKER_CONTACT_GROUPS,
				COMPONENT_CB_GROUPS);
		return rowFormat;
	}
	...
	/**
	 * @param textFieldToBeSet
	 *            Thinlet textfield whose value will be set with the selected
	 *            file
	 * @see FrontlineUI#showOpenModeFileChooser(Object)
	 */
	public void showOpenModeFileChooser() {
		FileChooser fc = FileChooser.createFileChooser(this.uiController, this,
				"openChooseComplete");
		fc.setFileFilter(new FileNameExtensionFilter(
				"FrontlineSMS Exported Data (" + CsvExporter.CSV_EXTENSION
						+ ")", CsvExporter.CSV_FORMAT));
		fc.show();
	}
	...
	public void saveChooseComplete(String filename) {
		uiController.setText(
				uiController.find(this.wizardDialog, "tfFilename"), filename);
